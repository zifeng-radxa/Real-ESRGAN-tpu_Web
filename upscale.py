...
class Upscale():
    def __init__(self, input_path, output_path, tmp_path=None, num_worker=1):
    ...
    def model_inference(self, model, frame, t_name, tqdm_tool):
        for i in frame:
    ...
    def __call__(self, audio_check):
        ...
        for task_frame in each_task_frame:
            ...
            for index, worker in enumerate(self.worker):
                t = Thread(target=self.model_inference, args=(worker, self.frames[index], str(index), tqdm_tool))
